...
interface UrlInputProps {
  onSubmitAudio: (requests: AudioDownloadRequest[], quality: string) => void;
...
export function UrlInput({ onSubmitAudio, onSubmitVideo, settingsOpen, onOpenSettings, onCloseSettings }: UrlInputProps) {
  ...
  const [audioSelections, setAudioSelections] = useState<AudioSelection[] | null>(null);
      ...
      {audioSelections && (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4">
		  <div className="max-h-[90vh] w-full max-w-2xl space-y-4 overflow-y-auto rounded-xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-gray-700 dark:bg-gray-800 sm:p-5">
		      ...
		      <div key={selection.url} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-gray-700 dark:bg-gray-900/50 sm:flex-row">
		        ...
		        <div className="min-w-0 flex-1 space-y-2">
		          {selection.info?.title ? (
		            <>
		              <p className="break-words text-sm text-slate-800 dark:text-gray-200 sm:truncate">{selection.info.title}</p>
		              <a href={selection.url} target="_blank" rel="noreferrer" className="block break-all text-sm text-blue-600 underline-offset-2 hover:underline dark:text-blue-400 sm:truncate">{selection.url}</a>
		            </>
		          ) : (
		            <a href={selection.url} target="_blank" rel="noreferrer" className="block break-all text-sm text-blue-600 underline-offset-2 hover:underline dark:text-blue-400 sm:truncate">{selection.url}</a>
		          )}
		          {selection.error && <p className="text-sm text-red-600 dark:text-red-400">{selection.error}</p>}
		          {!selection.info && !selection.error && <p className="text-sm text-slate-500 dark:text-gray-400">Carregando fontes de audio...</p>}
		          {selection.info && (!selection.info.audio_formats || selection.info.audio_formats.length === 0) && <p className="text-sm text-red-600 dark:text-red-400">Nenhuma fonte de audio disponivel.</p>}
		          {selection.info && selection.info.audio_formats && selection.info.audio_formats.length > 0 && (
		            <select
		              value={selection.selectedFormatID || ''}
		              onChange={(event) => setAudioSelections(current => current?.map(value => value.url === selection.url
		                ? { ...value, selectedFormatID: event.target.value }
		                : value) || null)}
		              className="app-select w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
		            >
		              {selection.info.audio_formats.map(format => (
		    ...
		    <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
		      <button type="button" onClick={() => setAudioSelections(null)} className="w-full rounded-lg border border-slate-300 bg-slate-100 px-4 py-2 text-sm text-slate-800 hover:bg-slate-200 dark:border-gray-500 dark:bg-gray-700/90 dark:text-gray-100 dark:hover:bg-gray-600 sm:w-auto">Cancelar</button>
		      <button
		        type="button"
		        disabled={audioSelections.some(selection => !selection.info || !selection.selectedFormatID)}
		        onClick={() => {
		          const requests = audioSelections.flatMap(selection => {
